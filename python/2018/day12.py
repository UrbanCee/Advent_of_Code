...
print("Task1: ",sum([(ci in {"#","X"})*(i-startIndex) for i,ci in enumerate(current)]))
for i in range(979):
    current=nextStep(current)
currentSum=sum([(ci in {"#","X"})*(i-startIndex) for i,ci in enumerate(current)])
current=nextStep(current)
difference=sum([(ci in {"#","X"})*(i-startIndex) for i,ci in enumerate(current)])-currentSum
print("Task2: ",sum([(ci in {"#","X"})*(i-startIndex) for i,ci in enumerate(current)])+(50000000000-1000)*difference)
